...
export class CreditPage {
   ...
   */
  cardNumberChange(){  
    let isNumber= /^\d+$/.test(this.cardnumber);
  ...
  saveCard(){
    ...
    this.paymentprovider.addCardInfo(this.cardinfor).subscribe( 
        data=>{
            console.log(data);
            loading.dismiss();
            if(data.json().msg=="success"){ 
                
                this.presentToast("Card has been added successfully",0);
            }else{
                this.presentToast("Invalid Card",1);
            }
        },
        err=>{
            console.log(err);
  ...
  save(){
    ...
    this.cardinfor.day=this.expirydate;
    this.cardinfor.password=this.password;
    this.cardinfor.country=this.countryname;

    this.stripe.setPublishableKey('pk_test_ZTkd7MUja1qvxVcDWtk1XmcC');
    ...
    this.paymentprovider.addpayment(this.cardinfor).subscribe(data=>{
      let res = data.json().success;
      if(!res){
        console.log("not saved your payment info");
      }
    });
